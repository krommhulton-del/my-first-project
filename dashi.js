...
(function (root, factory) {
  ...
  function yearEvidence(chart, gz, dayunGz, opts) {
    ...
    if (gCls === '官杀' || zCls === '官杀') {
      ...
      add('shiye', t === '忌' ? 1.6 : 2,
        t === '忌' ? '今年当值的是压着你的那股力,而这一年它是耗你的——硬仗、期限、说一不二的人都往你这边压'
    ...
    if ((gCls === '比劫' || zCls === '比劫') && (gTag === '忌' || zTag === '忌')) add('caiyun', 2, '今年跟你分东西的那股力当值:合伙、借贷、分账这类事最易漏财,先小人后君子', -1, '最容易在熟人身上破财:借钱、担保、合伙这三样这一年一概免谈,非做不可就写合同', 0, '比劫夺财:合伙、借贷、分账这类事最易漏财,先小人后君子');
    ...
    if (chart.gender === '男' && (gCls === '官杀' || zCls === '官杀')) add('zinv', 1.4, '今年当值的正是古法看男方子女的那条力——压着你的那股力那一路', 1, null, 0, '官杀当值——男命古法以官杀为子息');
    ...
    if ((gCls === '官杀' || zCls === '官杀') && (gTag === '忌' || zTag === '忌') && weak) add('jiankang', 2, '今年压着你的那股力正旺,而你本身这一年撑不住:劳损、旧疾、免疫这一路最先出问题', -1, '压力先伤睡眠和肠胃:这一年把体检排进日程,别拿硬扛当本事', 0, '七杀官星为忌又逢你身弱:压力攻身,劳损、旧疾、免疫这一路最先出问题');
    ...
    // ——— 是非官非 ———
    if ((gCls === '官杀' || zCls === '官杀') && isYangren) add('guanfei', 2, '压着你的那股力撞上一股戾气:争执升级之象,忍口舌、别硬顶', -1, '争执最容易升级成事故:这一年少表态、少接茬,该低头就低头', 0, '官杀逢羊刃:争执升级之象,忍口舌、别硬顶');
