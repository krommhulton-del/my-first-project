...
(function (root, factory) {
  ...
  // 旬空之义,出处:《增删卜易·兩現章第三十二》「凡占近病,用神得遇旬空者,不拘日月動爻克害,
  //   用神出空之日卽愈」;同处并载「元神動於卦中,用神空者,出空之日而得;忌神動於卦中,而用神空者,
  //   出空之日逢殃」——本程序的应期取「出空/实空之日」即依此(见 yingqi.js)。
  // **v0.79 更正**:此前这两句标的是「旬空章第二十六」。查下来那一章在本转录里**只有章名、没有正文**
  // (卷之一整卷如此),而这两句实际落在卷之二后段的兩現章。章名挂错了,照实改过来。
  // 新增 tools/chapter-check.mjs 专查这类「引文在书里搜得到、却不出自所标那一章」的错。
  ...
  // **原文把十四对列死了,照抄即可,不必自己推。**(上面那段引文即出自该章,此处不再重复挂书名。)
  ...
  // ——— 用神取法:问什么事,取哪一门六亲 ———
  //
  // **v0.79 把这张表的出处逐条查了一遍,结果一半能挂、一半挂不上,照实分开标(队列第 4 条的尾巴)。**
  //
  // 先说一件要紧的:v0.79 时这里挂着「用神章第八」当总纲出处——**那时挂不住**。
  // 老转录(增删卜易.txt)的**卷之一(第一章到第二十六章)只有章名、没有正文**,用神章第八正在其中;
  // 更巧的是原书自己在疾病章里写「占父母弟兄取用神者皆在用神章內詳之」,
  // 即逐事细目本就编在那一章——而那一章当时手上是空的。所以总纲那句删了,
  // 能挂的只有散在卷之三、卷之四各门类章里的逐事断语,三行只能写「待核」或「只挂书不挂章」。
  //
  // **v0.88 收尾:卷之一到手,三行全补上了。** 用户 2026-08-02 传进两样东西:
  //   · 增删卜易-维基文库本.txt——同一本书的第二份转录,恰是卷之一(32 章逐章有正文,用神章第八在内);
  //   · 卜筮正宗.txt 全文(简体)——其「十八论·用神分类定例第一」把五类六亲的用神逐条写死。
  // 据此:功名官非、文书房产两行从「只挂书不挂章」升为《增删卜易·用神章第八》的明文;
  // 病症取官鬼那半,《增删卜易》卷之一没写死,而《卜筮正宗》有明文,照挂(引文照它的简体原貌);
  // 兄弟为用那行两本书都有明文,与《增删卜易》序言「占朋友、外人以應爻爲用神」的分工记在行内。
  // 归章核对认得两份转录(同一本书,哪份核得到按哪份归章):tools/chapter-check.mjs。
  const YONG_SHEN = [
    // 次序要紧:先认死指向(老公/男友→官鬼、老婆/女友→妻财),再认事类,最后才按性别定婚恋。
    { re: /(老公|丈夫|男朋友|男友|男方|未婚夫|夫君)/, liuQin: '官鬼', note: '问夫婿以官鬼为用神',
      src: '出处:《增删卜易·隨鬼入墓章第三十》「妻占夫﹐亥水官星爲用神」' },
    { re: /(老婆|妻子|女朋友|女友|女方|未婚妻)/, liuQin: '妻财', note: '问妻室以妻财为用神',
      src: '出处:《增删卜易·夫婦章第四十三》「占妻財爻爲重﹐應爻次之」' },
    { re: /(病|疾|症|痛|健康|手术|住院|化验|癌|灾)/, liuQin: '官鬼', note: '问病以官鬼为病症、子孙为医药,两头对看',
      src: '出处:《卜筮正宗·用神分类定例第一》「凡占功名、官府、雷电、鬼神、丈夫、夫之兄弟同辈，及夫之相与朋友、乱臣、盗贼、邪崇、忧疑、病症、尸首、逆风，俱以官鬼爻为用神」' +
           '(病症那半;该转录是简体,引文照原貌)。医药那半,出处:《增删卜易·用神章第八》「占忠臣、良將、醫藥、僧、道、兵卒皆以子孫爻爲用神」' },
    { re: /(工作|职位|事业|升职|晋升|考公|考编|上班|老板|领导|官司|诉讼|名声|竞选|比赛|录取|面试)/, liuQin: '官鬼', note: '问功名事业官非以官鬼为用神',
      src: '出处:《增删卜易·用神章第八》「占功名、官府、雷霆鬼神、妻占夫皆以官鬼爻爲用神」(v0.79 只能挂书不挂章的那句「占官差」从此退为旁证)' },
    { re: /(财|钱|收入|工资|薪|生意|买卖|投资|理财|讨债|价格|赚|利润|报酬)/, liuQin: '妻财', note: '问钱财买卖以妻财为用神',
      src: '出处:《增删卜易·求財章第六十八》「公私占卜皆以財爲用神」' },
    { re: /(房|屋|宅|车|文书|合同|证件|学业|考试|论文|证书|长辈|父母|爸|妈|保险|执照)/, liuQin: '父母', note: '问房车文书学业长辈以父母为用神',
      src: '出处:《增删卜易·用神章第八》「占天地、城池、墻垣，宅捨、屋宇，舟車、衣服、雨具、綢緞，布匹、章奏、文書及書館文契也以父母爻爲用神」' },
    { re: /(子女|孩子|儿子|女儿|怀孕|备孕|生育|宠物|下属|平安|出行安否|保平安)/, liuQin: '子孙', note: '问子女平安以子孙为用神',
      src: '出处:《增删卜易·飛伏神章第二十八》「如占子孫﹐取子孫爲用神」' },
    // v0.79 在老转录全书里搜不到「兄弟为用」的明文,只能写待核;v0.88 两本书的明文都到了。
    { re: /(兄弟|姐妹|朋友|同事|合伙|同行|竞争对手)/, liuQin: '兄弟', note: '问平辈合伙以兄弟为用神',
      src: '出处:《增删卜易·用神章第八》「占兄弟、姐妹、族中兄弟、姑姨、妹夫及結拜兄弟都以兄弟爻爲用神」。同事合伙那半,' +
           '出处:《卜筮正宗·用神分类定例第一》「凡占兄弟、姊妹、姊妹丈、妻之兄弟、世兄弟、结盟同寅，及知交朋友，俱以兄弟爻为用神」——同寅即同僚。' +
           '另记一笔分工,出处:《增删卜易·序》「占朋友、外人以應爻爲用神」且野鹤自注「往往多有不驗」——' +
           '两书合看:深交的平辈按兄弟爻,没深交的生人外人按应爻;本行取兄弟,指的是知交平辈' },
  ];
  ...
  function yongShenOf(question, gender) {
    ...
    if (hits.length) {
      ...
      return {
        liuQin: main.liuQin, note: main.note, src: main.src,
        also: also.map(h => ({ liuQin: h.liuQin, note: h.note, src: h.src })),
        multi: also.length > 0,
      };
    }
    if (MARRIAGE_RE.test(q)) {
      // 泛指婚恋的两条,出处与上面点名夫/妻的那两条是同一处
