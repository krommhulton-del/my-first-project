...
await t('定时辰板块:分组、回推、能不能定、写回档案', async () => {
  ...
  // v1.17:此前这一页只摆参数,「这个差别意味着什么」一行都没有——
  // 而那些区间里可能藏着两套正好相反的结论。现在必须把「已经定了的」与「定不下来的」分开说。
  ok(/改 变 了 什 么/.test(txt), '缺「这几段的差别改变了什么」那一段');
  ok(/都已经定了|现在就能用/.test(txt), '要先说不管几点生都已经定了的那几条');
  ok(/定不下来|没定就定不下来/.test(txt), '要说清定不下来的是哪几条');
  ok(txt.indexOf('改 变 了 什 么') < txt.indexOf('还 站 得 住 的 区 间'), '这一段要摆在参数堆前面');
  // v1.12:诚实声明从口号(「自拟、零回测」)改成**摆实测数字**——18.9% 命中 vs 3.0% 碰运气,
  // 并写明慢星那一路不用来定生辰。数字比口号硬,钉子跟着换。
  ok(/18\.9%/.test(txt), '自测命中率必须印在界面上');
  ok(/不(用来|拿它)定生辰/.test(txt), '必须写明慢星那一路不用来定生辰');
  {
