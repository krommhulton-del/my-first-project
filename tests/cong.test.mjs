...
// v1.01:这条与 wangshuai 那条是同一个闸门的两处写法,一起放到 6.5%。
// **不是为了变绿**——是分格数变了:按《滴天髓阐微·从象章》原文补了「从气格」之后
// 真从由三格变四格(从弱/从强/从气),占比 3.72%→5.77%。上界是本项目自拟的经验闸,
// 每加一格都要重新量、重新写明;超过 6.5% 照旧当场红。
t('人群实测:真从(从弱+从强+从气)占比不超过 6.5%', () => {
  let n = 0, zhen = 0;
  ...
  ok(pct < 6.5, '真从占比=' + pct.toFixed(2) + '%(四格合计,上界 6.5% 是自拟经验闸)');
