...
(function (root, factory) {
  ...
  const DOMAIN = {
    ...
    官杀: { area: '工作 · 压力', good: '升迁、被赏识、接下重担,考试考评有利。', goodFem: '升迁、被赏识、接下重担,考试考评有利;女命另主感情桃花与正缘。', bad: '压力大、被管束、易惹是非官非,注意身体别硬扛,少跟人对着干。' },
  ...
  function judgeCard(chart, gan, zhi, label, span, opts) {
    ...
    if (gJ === '喜' && zJ === '忌') lines.push(`${P2}明面帮你、底下拆台——开头顺、后头漏,开场的甜头别全当真`);
    else if (gJ === '忌' && zJ === '喜') lines.push(`${P2}明面压你、底下托你——开头紧、后头稳,熬过开场有后劲`);
    ...
    lines.push(aOf(shen) === aOf(zShen)
      ? `${P2}明面和底下作用在同一类事上:${aOf(shen)}——这一类事会格外突出`
      : `${P2}明面上主要是${aOf(shen)},底下暗中走的是${aOf(zShen)}`);
    ...
    // 性别口径不许镜像(v0.95 定的规矩):「女命另主感情桃花」这半句只给女命。
    // 此前它无条件写死在 good 里,男命也照印——v1.01 改从格口径后有盘走到这条分支,才被测试照出来。
    const domainText = score >= 0
      ? ((dom.goodFem && chart.gender === '女') ? dom.goodFem : dom.good)
      : dom.bad;
    ...
    return {
      ...
      text: (hl
        ? `${{ 日: '今天', 月: '这个月', 年: '今年' }[one] || '这段'}重点在${dom.area},${hl.text}`
        : `${{ 日: '今天', 月: '这个月', 年: '今年' }[one] || '这段'}重点在${dom.area}。`)
